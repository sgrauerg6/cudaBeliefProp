...
if __name__ == "__main__":
	...
	imageSets = [conesImageSetQuarterSize]#[tsukubaImageSet, venusImageSet, barn1ImageSet, conesImageSetQuarterSize, conesImageSetHalfSize]
	...
	startRegisterMemoryOptions = [0]#range(98, 129, 2)
	localMemoryOptions = [0]
	numDispValsSet = [128] #range(1, 129, 1)
	numRuns = 0
										...
										for cpuOptimizationSetting in cpuOptimizationSettings:
											for memoryAlignmentOptimization in memoryAlignmentOptimizations:
												for startRegMemOption in startRegisterMemoryOptions:
													for currentLocalMemoryOption in localMemoryOptions:	
														for currentNumDispVals in numDispValsSet:
															#uncomment if using multiple disparity values on single image	
															...
															file.write("#define USE_SHARED_MEMORY_FROM_PYTHON %s\n" % currentLocalMemoryOption)
