...
namespace bp_simd_processing
{
	...
	//function retrieve the minimum value at each 1-d disparity value in O(n) time using Felzenszwalb's method (see "Efficient Belief Propagation for Early Vision")
	void dtStereoSIMD(float32x4_t* f, const unsigned int bpSettingsDispVals)
	{
		float32x4_t prev;
		float32x4_t vectorAllOneVal = vdupq_n_f32(1.0f);
	...
	void msgStereoSIMD(const unsigned int xVal, const unsigned int yVal, const levelProperties& currentLevelProperties,
			float32x4_t* messageValsNeighbor1, float32x4_t* messageValsNeighbor2,
			float32x4_t* messageValsNeighbor3, float32x4_t* dataCosts,
			float* dstMessageArray, const float32x4_t& disc_k_bp, const bool dataAligned,
			const unsigned int bpSettingsDispVals)
	{
		...
		//T minimum = INF_BP;
		float32x4_t minimum = vdupq_n_f32(bp_consts::INF_BP);
		float32x4_t* dst = new float32x4_t[bpSettingsDispVals];
		...
		//valToNormalize /= DISP_VALS;
		valToNormalize = vdivq_f32(valToNormalize, vdupq_n_f32((float)bpSettingsDispVals));
			...
			const unsigned int bpSettingsDispVals)
	{
		...
		//T minimum = INF_BP;
		float32x4_t minimum = vdupq_n_f32(bp_consts::INF_BP);
		float32x4_t* dstFloat = new float32x4_t[bpSettingsDispVals];
