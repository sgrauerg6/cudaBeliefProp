...
void convertUnsignedIntImageToFloatCPU(unsigned int* imagePixelsUnsignedIntToFilter, float* floatImagePixels, int widthImages, int heightImages)
{
...
void filterFloatImageAcrossCPU(float* imagePixelsFloatToFilter, float* filteredImagePixels, int widthImages, int heightImages, float* imageFilter, int sizeFilter)
{
	#pragma omp parallel for
	for (int val = 0; val < widthImages*heightImages; val++)
	{
				...
				for (int i = 1; i < sizeFilter; i++) {
					filteredPixelVal += imageFilter[i]
							* (imagePixelsFloatToFilter[yVal * widthImages
									+ std::max(xVal - i, 0)]
									+ imagePixelsFloatToFilter[yVal
											* widthImages
											+ std::min(xVal + i, widthImages - 1)]);
				}

				filteredImagePixels[yVal * widthImages + xVal] =
						filteredPixelVal;
			}
		}
	//}
...
//input image stored in texture imagePixelsFloatToFilterTexture
//output filtered image stored in filteredImagePixels
void filterFloatImageVerticalCPU(float* imagePixelsFloatToFilter, float* filteredImagePixels, int widthImages, int heightImages, float* imageFilter, int sizeFilter)
{
	#pragma omp parallel for
	for (int val = 0; val < widthImages*heightImages; val++)
	{
	...
	/*for (int yVal = 0; yVal < heightImages; yVal++)
	{
		...
		{*/
			...
			//		heightImages))
			{
				...
				for (int i = 1; i < sizeFilter; i++) {
					filteredPixelVal += imageFilter[i]
							* (imagePixelsFloatToFilter[std::max(yVal - i, 0)
									* widthImages + xVal]
									+ imagePixelsFloatToFilter[std::min(yVal + i,
											heightImages - 1) * widthImages
											+ xVal]);
				}
...
void filterUnsignedIntImageAcrossCPU(unsigned int* imagePixelsUnsignedIntToFilter, float* filteredImagePixels, int widthImages, int heightImages, float* imageFilter, int sizeFilter)
{
	#pragma omp parallel for
	for (int val = 0; val < widthImages*heightImages; val++)
	{
...
//the input image is stored as unsigned ints in the texture imagePixelsUnsignedIntToFilterTexture
//the output filtered image is returned as an array of floats
void filterUnsignedIntImageVerticalCPU(unsigned int* imagePixelsUnsignedIntToFilter, float* filteredImagePixels, int widthImages, int heightImages, float* imageFilter, int sizeFilter)
{
	#pragma omp parallel for
	for (int val = 0; val < widthImages*heightImages; val++)
	{
...
void SmoothImageCPU::operator()(unsigned int* inImage, unsigned int widthImages, unsigned int heightImages, float sigmaVal, float* smoothedImage)
{
	...
	if (sigmaVal < MIN_SIGMA_VAL_SMOOTH)
	{
		...
		convertUnsignedIntImageToFloatCPU(inImage,
				smoothedImage, widthImages, heightImages);
	}
	...
	else
	{
		...
		filterUnsignedIntImageAcrossCPU(inImage,
				intermediateImage, widthImages, heightImages, filter,
				sizeFilter);

		//now use the vertical filter to complete the smoothing of image 1 on the device
		filterFloatImageVerticalCPU(intermediateImage,
				smoothedImage, widthImages, heightImages, filter,
				sizeFilter);
