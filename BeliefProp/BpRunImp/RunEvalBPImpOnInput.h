...
protected:
  ...
  std::shared_ptr<ParallelParams> setUpParallelParams(const run_environment::RunImpSettings& runImpSettings) override {
  ...
  //TODO: need to add support for running both templated and non templated disparity value count implementations
  std::pair<run_eval::Status, RunData> runImpsAndCompare(
    std::shared_ptr<ParallelParams> parallelParams, bool runOptImpOnly, bool runImpTmpLoopIters) const override
  { 
