	...
	for (int numRun=0; numRun < NUM_BP_STEREO_RUNS; numRun++)
{
	...
	unsigned int* image1AsUnsignedIntArrayHost = loadImageFromPGM(imageFiles[0], widthImages, heightImages);
	unsigned int* image2AsUnsignedIntArrayHost = loadImageFromPGM(imageFiles[1], widthImages, heightImages);
...
//(cutCreateTimer(&timerIncludeTransferTime));
	//(cutCreateTimer(&timerTransferTimeNotIncluded));

	//allocate the device memory to store and x and y smoothed images
	(cudaMalloc((void**) &smoothedImage1Device, widthImages * heightImages * sizeof(float)));
	(cudaMalloc((void**) &smoothedImage2Device, widthImages * heightImages * sizeof(float)));

	//declare the allocate the space for image 1 and image 2 in the device
	unsigned int* image1Device;
	unsigned int* image2Device;

	cudaMalloc((void **) &image1Device, widthImages * heightImages * sizeof(unsigned int));
	cudaMalloc((void **) &image2Device, widthImages * heightImages * sizeof(unsigned int));

	//start timer to retrieve the time of implementation including transfer time
	//CUT_SAFE_CALL(cutStartTimer(timerIncludeTransferTime));
    gettimeofday(&timeWithTransferStart, NULL);

	//transfer the image 1 and image 2 data from the host to the device
	cudaMemcpy(image1Device, image1AsUnsignedIntArrayHost, widthImages * heightImages * sizeof(unsigned int), cudaMemcpyHostToDevice);
	cudaMemcpy(image2Device, image2AsUnsignedIntArrayHost, widthImages * heightImages * sizeof(unsigned int), cudaMemcpyHostToDevice);
	...
	//cutStartTimer(timerTransferTimeNotIncluded);
	...
	smoothImagesAllDataInDevice(image1Device, image2Device, widthImages, heightImages, SIGMA_BP, smoothedImage1Device, smoothedImage2Device);
	...
	cudaMalloc((void **) &disparityMapFromImage1To2Device, widthImages * heightImages * sizeof(float));



	//run BP on the image if image is smaller than "chunk size"
	if ((widthImages <= WIDTH_IMAGE_CHUNK_RUN_STEREO_EST_BP) && (heightImages <= HEIGHT_IMAGE_CHUNK_RUN_STEREO_EST_BP))
	{
		printf("RUN FULL IMAGE\n");
		runBeliefPropStereoCUDA(smoothedImage1Device, smoothedImage2Device, disparityMapFromImage1To2Device, algSettings);
	}
	//otherwise run the BP Stereo on the image set "in chunks"
	else
	{
		printf("RUN IMAGE IN CHUNKS\n");
		runBPStereoEstOnImageSetInChunks(smoothedImage1Device, smoothedImage2Device, widthImages, heightImages, disparityMapFromImage1To2Device, algSettings);
	}
	...
	double timeStart = timeNoTransferStart.tv_sec+(timeNoTransferStart.tv_usec/1000000.0);
	double timeEnd = timeNoTransferEnd.tv_sec+(timeNoTransferEnd.tv_usec/1000000.0);
printf("Running time not including transfer time: %.10lf seconds\n", timeEnd-timeStart);
	...
	if (saveResults)
	{
		saveResultingDisparityMap(saveDisparityMapImagePaths[0], disparityMapFromImage1To2Device, SCALE_BP, widthImages, heightImages, timeWithTransferStart);
	}
	...
	for (int numImage = 2; numImage < numImages; numImage++)
	{
		...
		cudaMemcpy(smoothedImage1Device, smoothedImage2Device, widthImages * heightImages * sizeof(float), cudaMemcpyDeviceToDevice);


		//load the next image from memory...this is will be image 2
		image2AsUnsignedIntArrayHost = loadImageFromPGM(imageFiles[numImage], widthImages, heightImages);


		//start timer for implementation including transfer time
		//cutStartTimer(timerIncludeTransferTime);

		//transfer the image from the host to the device
		cudaMemcpy(image2Device, image2AsUnsignedIntArrayHost, widthImages * heightImages * sizeof(float), cudaMemcpyHostToDevice);
		...
		smoothSingleImageAllDataInDevice(image2Device, widthImages, heightImages, SIGMA_BP, smoothedImage2Device);
		...
		if (saveResults)
		{
			saveResultingDisparityMap(saveDisparityMapImagePaths[numImage - 1], disparityMapFromImage1To2Device, SCALE_BP, widthImages, heightImages, timeWithTransferStart);
		}
