...
//half type gets converted to float for processing and then back to half for
//storage
template <typename T>
...
concept RunDataVect_t =
  ...
  std::is_same_v<T, __m512h> ||
  std::is_same_v<T, __m256h>;
...
template <typename T>
concept RunDataSingOrVect_t =
  RunData_t<T> ||
  RunDataVect_t<T>;

template <typename T>
concept RunDataProcessSingOrVect_t =
  RunDataProcess_t<T> ||
  RunDataVectProcess_t<T>;
