...
/**
 * @brief Structure with output disparity map, runtime, and other evaluation
 * data
 */
struct BpRunOutput
{
...
/**
 * @brief Structure with pointers to objects containing functions for smoothing
 * images, processing bp, and memory management on target device using
 * specified acceleration
 * 
 * @tparam T 
 * @tparam DISP_VALS 
...
/**
 * @brief Abstract class to set up and run belief propagation on target device
  ...
  const beliefprop::BpOnDevice<T, DISP_VALS, ACCELERATION>& run_bp_on_device) const
{
  ...
  const std::array<unsigned int, 2> width_height_images{
    ...
    input_images[0].Height()};

  //get total number of pixels in input images
  const unsigned int tot_num_pixels_images{
    width_height_images[0] * width_height_images[1]};

  //initialize structures for timing data
  std::unordered_map<beliefprop::Runtime_Type, std::array<timingType, 2>>
  runtime_start_end_timings;
  ...
  const unsigned int num_evaluation_runs{
    beliefprop::NumBpStereoRuns(alg_settings.num_disp_vals)};
       ...
       num_run++)
  {
    ...
    std::array<float*, 2> smoothed_images{
      run_bp_on_device.mem_management_images->AllocateMemoryOnDevice(
        tot_num_pixels_images),
      run_bp_on_device.mem_management_images->AllocateMemoryOnDevice(
        tot_num_pixels_images)};
    
    if (run_bp_on_device.run_bp_stereo->ErrorCheck(__FILE__, __LINE__) !=
        run_eval::Status::kNoError)
    {
    ...
    runtime_start_end_timings[beliefprop::Runtime_Type::kSmoothing][0] =
      std::chrono::system_clock::now();
    runtime_start_end_timings[beliefprop::Runtime_Type::kTotalNoTransfer][0] =
      std::chrono::system_clock::now();
    runtime_start_end_timings[beliefprop::Runtime_Type::kTotalWithTransfer][0] =
      std::chrono::system_clock::now();
    ...
    for (unsigned int i = 0; i < 2; i++) {
      (*(run_bp_on_device.smooth_image))(
        input_images[i],
        alg_settings.smoothing_sigma,
        smoothed_images[i]);
      if (run_bp_on_device.run_bp_stereo->ErrorCheck(__FILE__, __LINE__) !=
          run_eval::Status::kNoError)
      { 
    ...
    runtime_start_end_timings[beliefprop::Runtime_Type::kSmoothing][1] =
      std::chrono::system_clock::now();

    //get and store time point at start of bp processing
    runtime_start_end_timings[beliefprop::Runtime_Type::kTotalBp][0] =
      std::chrono::system_clock::now();
    ...
    auto bp_stereo_output =
      (*(run_bp_on_device.run_bp_stereo))(
        ...
        bp_data,
        bp_proc_store,
        run_bp_on_device.mem_management_bp_run);
    if (!bp_stereo_output) {
      //return std::optional object with no value if null output indicating
      //error in run
      return {};
    }
    ...
    //transfer the disparity map estimation on the device to the host for
    //output
    run_bp_on_device.mem_management_images->TransferDataFromDeviceToHost(
      output_disparity_map.PointerToPixelsStart(),
      bp_disparity_map,
      tot_num_pixels_images);
    ...
    //free the space allocated to the resulting disparity map and smoothed
    //images on the computation device
  ...
  //construct and return beliefprop::BpRunOutput object inside of std::optional
  //object
