...
public:
  ...
  static std::size_t TotalDataForAlignedMemoryAllLevels(const std::array<unsigned int, 2>& width_height_bottom_level,
    unsigned int num_possible_disparities, unsigned int num_levels, run_environment::AccSetting acceleration);
...
BpLevel<T> BpLevel<T>::NextBpLevel(unsigned int num_disparity_values) const {
  const std::size_t offset_next_level = level_properties_.offset_into_arrays_ + NumDataInBpArrays(num_disparity_values);
  return BpLevel<T>({(unsigned int)std::ceil((float)level_properties_.width_level_ / 2.0f),
...
std::size_t BpLevel<T>::NumDataInBpArrays(unsigned int num_disparity_values) const {
  return NumDataForAlignedMemoryAtLevel({level_properties_.width_level_, level_properties_.height_level_}, num_disparity_values);
}
  ...
  unsigned int num_possible_disparities, unsigned int num_levels, run_environment::AccSetting acceleration)
{
  ...
  std::size_t total_data = curr_level_properties.NumDataInBpArrays(num_possible_disparities);
...
template <typename T>
BpLevel<T>::BpLevel(const std::array<unsigned int, 2>& width_height, std::size_t offset_into_arrays, unsigned int level_num,
...
template <typename T>
unsigned int BpLevel<T>::CheckerboardWidthTargetDevice(unsigned int width_level) const {
...
unsigned int BpLevel<T>::PaddedCheckerboardWidth(unsigned int checkerboard_width) const
{
  ...
  return ((checkerboard_width % num_data_align_width) == 0) ?
         checkerboard_width :
         (checkerboard_width + (num_data_align_width - (checkerboard_width % num_data_align_width)));
}
