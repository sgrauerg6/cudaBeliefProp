  ...
  size_t data_type_size) const 
{
  ...
  else if (this->opt_imp_accel_ == run_environment::AccSetting::kHIP) {
    return RunEvalImpMultDataSets<run_environment::AccSetting::kHIP>(run_imp_settings, data_type_size);
  }
  else if (this->opt_imp_accel_ == run_environment::AccSetting::kMETAL) {
  ...
  const run_environment::RunImpSettings& run_imp_settings) const
{
  ...
  run_results.push_back(RunImpOnInputBnchmrks<
    T, OPT_IMP_ACCEL, static_cast<size_t>(benchmarks::MtrxWH::kMtrxWH_32), benchmarks::BenchmarkRun::/*kAddTwoD*/kGemm>().operator()(
    run_imp_settings));
  run_results.push_back(RunImpOnInputBnchmrks<
  ...
  /*run_results.push_back(RunImpOnInputBnchmrks<
    ...
    run_imp_settings));
  run_results.push_back(RunImpOnInputBnchmrks<
    T, OPT_IMP_ACCEL, static_cast<size_t>(benchmarks::MtrxWH::kMtrxWH_8192), benchmarks::BenchmarkRun::kAddTwoD>().operator()(
    run_imp_settings));
  run_results.push_back(RunImpOnInputBnchmrks<
    T, OPT_IMP_ACCEL, static_cast<size_t>(benchmarks::MtrxWH::kMtrxWH_12288), benchmarks::BenchmarkRun::kAddTwoD>().operator()(
    run_imp_settings));
  run_results.push_back(RunImpOnInputBnchmrks<
  ...
  /*run_results.push_back(RunImpOnInputBnchmrks<
    ...
    run_imp_settings));*/
  /*run_results.push_back(RunImpOnInputBnchmrks<
    T, OPT_IMP_ACCEL, static_cast<size_t>(benchmarks::MtrxWH::kMtrxWH_24576), benchmarks::BenchmarkRun::kAddTwoD>().operator()(
    run_imp_settings));*/
