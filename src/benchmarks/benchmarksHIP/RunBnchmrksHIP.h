...
class RunBnchmrksHIP : public RunBnchmrks<T, ACCELERATION, BENCHMARK_RUN> {
public:
  std::string RunDescription() const override { return std::string(run_hip::kHIPDesc); }
...
{
  //return no value if acceleration setting is not HIP
  if constexpr (ACCELERATION != run_environment::AccSetting::kHIP) {
    return {};
  }

  //generate struct with pointers to objects for running HIP implementation and call
  //function to run HIP implementation
  RunData run_data;
  run_data.AppendData(run_hip::retrieveDeviceProperties(0));
