...
public:
  ...
  std::pair<MultRunData, std::vector<MultRunSpeedup>> RunDataWSpeedups() const;
...
private:
  ...
  void WriteRunOutput(const std::pair<MultRunData, std::vector<MultRunSpeedup>>& run_output, const run_environment::RunImpSettings& run_imp_settings,
    run_environment::AccSetting acceleration_setting, unsigned int data_type_size = 0) const;
  ...
  std::vector<MultRunSpeedup> GetSpeedupOverBaseline(const run_environment::RunImpSettings& run_imp_settings,
  ...
  std::vector<MultRunSpeedup> GetSpeedupOverBaselineSubsets(const run_environment::RunImpSettings& run_imp_settings,
    MultRunData& run_data_all_runs, size_t data_type_size) const;

  //get baseline runtime data if available...return null if baseline data not available
  std::optional<std::pair<std::string, std::vector<double>>> GetBaselineRuntimeData(std::string_view baseline_data_path) const;
  ...
  std::vector<MultRunSpeedup> GetAvgMedSpeedupOverBaselineSubsets(MultRunData& run_output,
    std::string_view data_type_str, const std::array<std::string_view, 2>& base_data_path_opt_single_thread,
    const std::vector<std::pair<std::string, std::vector<unsigned int>>>& subset_str_indices = std::vector<std::pair<std::string, std::vector<unsigned int>>>()) const;

  //get average and median speedup of current runs compared to baseline data from file
  std::vector<MultRunSpeedup> GetAvgMedSpeedupOverBaseline(MultRunData& run_output,
  ...
  MultRunSpeedup GetAvgMedSpeedupOptPParams(MultRunData& run_output, std::string_view speedup_header) const;

  //get average and median speedup between base and target runtime data
  MultRunSpeedup GetAvgMedSpeedup(MultRunData& run_output_base, MultRunData& run_output_target,
  ...
  MultRunSpeedup GetAvgMedSpeedupLoopItersInTemplate(MultRunData& run_output,
    std::string_view speedup_header) const;
  ...
  std::vector<MultRunSpeedup> run_imp_speedups_;
