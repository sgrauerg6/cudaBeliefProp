...
void EvaluateImpResults::operator()(const MultRunData& run_results, const run_environment::RunImpSettings run_imp_settings, run_environment::AccSetting opt_imp_acc, size_t data_size) {
...
std::pair<MultRunData, std::vector<MultRunSpeedup>> EvaluateImpResults::RunDataWSpeedups() const {
...
void EvaluateImpResults::WriteRunOutput(const std::pair<MultRunData, std::vector<MultRunSpeedup>>& run_output, const run_environment::RunImpSettings& run_imp_settings,
  run_environment::AccSetting acceleration_setting, unsigned int data_type_size) const
{
  //get iterator to first run with success
  const auto first_success_run = std::find_if(run_output.first.cbegin(), run_output.first.cend(), [](const auto& run_result)
    { return run_result; } );
  ...
  if (first_success_run != run_output.first.cend()) {
    ...
    const std::string data_type_str = kMultDataTypes ? "" : '_' + std::string(run_environment::kDataSizeToNameMap.at(data_type_size));
    const auto accel_str = kMultDataTypes ? "" : '_' + std::string(run_environment::accelerationString(acceleration_setting));
    ...
    const std::filesystem::path opt_results_file_path{imp_results_fp / run_eval::kImpResultsRunDataFolderName / std::filesystem::path(((run_imp_settings.runName_) ? std::string(run_imp_settings.runName_.value()) + "_" : "") + 
      std::string(run_eval::kRunResultsDescFileName) + data_type_str + accel_str + std::string(run_eval::kCsvFileExtension))};
    const std::filesystem::path opt_results_w_speedup_file_path{imp_results_fp / run_eval::kImpResultsRunDataWSpeedupsFolderName / std::filesystem::path(((run_imp_settings.runName_) ? std::string(run_imp_settings.runName_.value()) + "_" : "") + 
      std::string(run_eval::kRunResultsWSpeedupsDescFileName) + data_type_str + accel_str + std::string(run_eval::kCsvFileExtension))};
    const std::filesystem::path default_params_results_file_path{imp_results_fp / run_eval::kImpResultsRunDataFolderName / std::filesystem::path(((run_imp_settings.runName_) ? std::string(run_imp_settings.runName_.value()) + "_" : "") + 
      std::string(run_eval::kRunResultsDescDefaultPParamsFileName) + data_type_str + accel_str + std::string(run_eval::kCsvFileExtension))};
    const std::filesystem::path speedups_results_file_path{imp_results_fp / run_eval::kImpResultsSpeedupsFolderName / std::filesystem::path(((run_imp_settings.runName_) ? std::string(run_imp_settings.runName_.value()) + "_" : "") + 
      std::string(run_eval::kSpeedupsDescFileName) + data_type_str + accel_str + std::string(run_eval::kCsvFileExtension))};
    ...
    for (unsigned int i=0; i < (run_imp_settings.optParallelParamsOptionSetting_.first ? run_data_opt_default_sstr.size() : 1); i++) {
    ...
    for (unsigned int i=0; i < (run_imp_settings.optParallelParamsOptionSetting_.first ? run_data_opt_default_sstr.size() : 1); i++) {
      for (unsigned int run_num=0; run_num < run_output.first.size(); run_num++) {
        ...
        const unsigned int runResultIdx = run_output.first[run_num] ? i : 0;
        for (const auto& curr_header : headers_in_order) {
          if (!(run_output.first[run_num]->at(runResultIdx).IsData(curr_header))) {
            run_data_opt_default_sstr[i] << "No Data" << ',';
          }
          else {
            run_data_opt_default_sstr[i] << run_output.first[run_num]->at(runResultIdx).GetDataAsStr(curr_header) << ',';
          }
        }
        run_data_opt_default_sstr[i] << std::endl;
      }
    ...
    for (const auto& speedup_desc_w_index : {std::pair<std::string, size_t>{"Average Speedup", 0}, std::pair<std::string, size_t>{"Median Speedup", 1}}) {
    ...
    std::array<std::ofstream, 2> results_stream_default_tb_final{
      run_imp_settings.optParallelParamsOptionSetting_.first ? (write_debug_output_files_ ? std::ofstream(default_params_results_file_path) : std::ofstream()) : std::ofstream(opt_results_file_path),
      run_imp_settings.optParallelParamsOptionSetting_.first ? std::ofstream(opt_results_file_path) : std::ofstream()};
    std::ofstream run_result_w_speedup_sstr(opt_results_w_speedup_file_path);
    if (write_debug_output_files_ || (!run_imp_settings.optParallelParamsOptionSetting_.first)) {
    ...
    std::cout << "Input/settings/parameters info, detailed timings, and evaluation for each run and across runs in " << opt_results_w_speedup_file_path << std::endl;
    ...
    EvaluateAcrossRuns().operator()(imp_results_fp, GetCombResultsTopText(), GetInputParamsShow());
  }
  else {
...
void EvaluateImpResults::EvalResultsSingDTypeAccRun() {
  ...
  run_imp_opt_results_ = run_imp_orig_results_;
  const auto speedup_over_baseline = GetSpeedupOverBaseline(run_imp_settings_, run_imp_opt_results_, data_size_);
  const auto speedup_over_baseline_subsets = GetSpeedupOverBaselineSubsets(run_imp_settings_, run_imp_opt_results_, data_size_);
  run_imp_speedups_.insert(run_imp_speedups_.cend(), speedup_over_baseline.cbegin(), speedup_over_baseline.cend());
  run_imp_speedups_.insert(run_imp_speedups_.cend(), speedup_over_baseline_subsets.cbegin(), speedup_over_baseline_subsets.cend());
  ...
  if (run_imp_settings_.optParallelParamsOptionSetting_.first) {
    run_imp_speedups_.push_back(GetAvgMedSpeedupOptPParams(run_imp_opt_results_, std::string(run_eval::kSpeedupOptParParamsHeader) + " - " +
      std::string(run_environment::kDataSizeToNameMap.at(data_size_))));
  }
  if (run_imp_settings_.templatedItersSetting_ == run_environment::TemplatedItersSetting::kRunTemplatedAndNotTemplated) {
    run_imp_speedups_.push_back(GetAvgMedSpeedupLoopItersInTemplate(run_imp_opt_results_, std::string(run_eval::kSpeedupDispCountTemplate) + " - " +
      std::string(run_environment::kDataSizeToNameMap.at(data_size_))));
  }

  //write output corresponding to results for current data type if writing debug output
  if (write_debug_output_files_) {
    constexpr bool kMultDataTypes{false};
    WriteRunOutput<kMultDataTypes>({run_imp_opt_results_, run_imp_speedups_}, run_imp_settings_, opt_imp_accel_, data_size_);
  }
}
  ...
  const run_environment::RunImpSettings& run_imp_settings, size_t data_type_size, run_environment::AccSetting fastest_acc) const
{
  ...
  if (run_imp_results_by_acc_setting.size() == 1) {
    ...
    return {{acc_to_speedup_str.at(run_environment::AccSetting::kNone), {0.0, 0.0}}, {acc_to_speedup_str.at(run_environment::AccSetting::kAVX256), {0.0, 0.0}}};
  }
  else {
    //initialize speedup/slowdown using alternate acceleration
    std::vector<MultRunSpeedup> alt_acc_speedups;
      ...
      if ((alt_acc_imp_results.first != fastest_acc) && (acc_to_speedup_str.contains(alt_acc_imp_results.first))) {
        ...
        for (unsigned int i = 0; i < run_imp_results_by_acc_setting[fastest_acc].first.size(); i++) {
          if (run_imp_results_by_acc_setting[fastest_acc].first[i] && alt_acc_imp_results.second.first[i]) {
            const double init_result_time = run_imp_results_by_acc_setting[fastest_acc].first[i]->back().GetDataAsDouble(run_eval::kOptimizedRuntimeHeader).value();
            const double alt_acc_result_time = alt_acc_imp_results.second.first[i]->back().GetDataAsDouble(run_eval::kOptimizedRuntimeHeader).value();
...
void EvaluateImpResults::EvalResultsMultDTypeAccRuns() {
  ...
  std::unordered_map<size_t, std::vector<MultRunSpeedup>> alt_imp_speedup;
  std::unordered_map<size_t, MultRunSpeedup> alt_datatype_speedup;
  for (const size_t data_size : {sizeof(float), sizeof(double), sizeof(halftype)}) {
    alt_imp_speedup[data_size] = GetAltAccelSpeedups(run_imp_results_mult_runs_[data_size], run_imp_settings_, data_size, opt_imp_accel_);
    if (data_size != sizeof(float)) {
      //get speedup or slowdown using alternate data type (double or half) compared with float
      alt_datatype_speedup[data_size] = GetAvgMedSpeedup(run_imp_results_mult_runs_[sizeof(float)][opt_imp_accel_].first,
        run_imp_results_mult_runs_[data_size][opt_imp_accel_].first, (data_size > sizeof(float)) ? run_eval::kSpeedupDouble : run_eval::kSpeedupHalf);
    }
  }

  //initialize overall results to float results using fastest acceleration and add double and half-type results to it
  auto results_w_speedups = run_imp_results_mult_runs_[sizeof(float)][opt_imp_accel_];
  results_w_speedups.first.insert(results_w_speedups.first.cend(),
    run_imp_results_mult_runs_[sizeof(double)][opt_imp_accel_].first.cbegin(), run_imp_results_mult_runs_[sizeof(double)][opt_imp_accel_].first.cend());
  results_w_speedups.first.insert(results_w_speedups.first.cend(),
    run_imp_results_mult_runs_[sizeof(halftype)][opt_imp_accel_].first.cbegin(), run_imp_results_mult_runs_[sizeof(halftype)][opt_imp_accel_].first.cend());

  //add speedup data from double and half precision runs to speedup results
  results_w_speedups.second.insert(results_w_speedups.second.cend(), alt_imp_speedup[sizeof(float)].cbegin(), alt_imp_speedup[sizeof(float)].cend());
  results_w_speedups.second.insert(results_w_speedups.second.cend(),
    run_imp_results_mult_runs_[sizeof(double)][opt_imp_accel_].second.cbegin(), run_imp_results_mult_runs_[sizeof(double)][opt_imp_accel_].second.cend());
  results_w_speedups.second.insert(results_w_speedups.second.cend(), alt_imp_speedup[sizeof(double)].cbegin(), alt_imp_speedup[sizeof(double)].cend());
  results_w_speedups.second.insert(results_w_speedups.second.cend(),
    run_imp_results_mult_runs_[sizeof(halftype)][opt_imp_accel_].second.cbegin(), run_imp_results_mult_runs_[sizeof(halftype)][opt_imp_accel_].second.cend());
  results_w_speedups.second.insert(results_w_speedups.second.cend(), alt_imp_speedup[sizeof(halftype)].cbegin(), alt_imp_speedup[sizeof(halftype)].cend());
  ...
  if ((run_imp_settings_.baseOptSingThreadRTimeForTSetting_) &&
      (run_imp_settings_.baseOptSingThreadRTimeForTSetting_.value().second == run_imp_settings_.templatedItersSetting_)) {
      const auto speedup_over_baseline = GetAvgMedSpeedupOverBaseline(results_w_speedups.first, "All Runs", run_imp_settings_.baseOptSingThreadRTimeForTSetting_.value().first);
  ...
  if (run_imp_settings_.optParallelParamsOptionSetting_.first) {
    results_w_speedups.second.push_back(GetAvgMedSpeedupOptPParams(results_w_speedups.first, std::string(run_eval::kSpeedupOptParParamsHeader) + " - All Runs"));
  }

  //get speedup when using template for loop iteration count
  if (run_imp_settings_.templatedItersSetting_ == run_environment::TemplatedItersSetting::kRunTemplatedAndNotTemplated) {
    results_w_speedups.second.push_back(GetAvgMedSpeedupLoopItersInTemplate(results_w_speedups.first, std::string(run_eval::kSpeedupDispCountTemplate) + " - All Runs"));
  }

  //add speedups when using doubles and half precision compared to float to end of speedup data
  results_w_speedups.second.insert(results_w_speedups.second.cend(), {alt_datatype_speedup[sizeof(double)], alt_datatype_speedup[sizeof(halftype)]});

  //write output corresponding to results and speedups for all data types
  constexpr bool kMultDataTypes{true};
  WriteRunOutput<kMultDataTypes>(results_w_speedups, run_imp_settings_, opt_imp_accel_);
}

//get speedup over baseline data if data available
std::vector<MultRunSpeedup> EvaluateImpResults::GetSpeedupOverBaseline(const run_environment::RunImpSettings& run_imp_settings,
  MultRunData& run_data_all_runs, size_t data_type_size) const
{
  //initialize speedup results
  std::vector<MultRunSpeedup> speedup_results;
  ...
  if (data_type_size == sizeof(float)) {
    //get speedup over baseline runtimes...can only compare with baseline runtimes that are
    //generated using same templated iterations setting as current run
    if ((run_imp_settings.baseOptSingThreadRTimeForTSetting_) &&
        (run_imp_settings.baseOptSingThreadRTimeForTSetting_.value().second == run_imp_settings.templatedItersSetting_))
    {
      const auto speedup_over_baseline_subsets = GetAvgMedSpeedupOverBaseline(
        ...
        run_imp_settings.baseOptSingThreadRTimeForTSetting_.value().first);
...
std::vector<MultRunSpeedup> EvaluateImpResults::GetSpeedupOverBaselineSubsets(const run_environment::RunImpSettings& run_imp_settings,
  MultRunData& run_data_all_runs, const size_t data_type_size) const
{
  if ((data_type_size == sizeof(float)) &&
    (run_imp_settings.baseOptSingThreadRTimeForTSetting_ && (run_imp_settings.baseOptSingThreadRTimeForTSetting_->second == run_imp_settings.templatedItersSetting_)))
  {
    return GetAvgMedSpeedupOverBaselineSubsets(run_data_all_runs, run_environment::kDataSizeToNameMap.at(data_type_size),
      run_imp_settings.baseOptSingThreadRTimeForTSetting_->first, run_imp_settings.subset_str_indices_);
  }
...
std::optional<std::pair<std::string, std::vector<double>>> EvaluateImpResults::GetBaselineRuntimeData(std::string_view baseline_data_path) const {
  std::ifstream baseline_data(std::string{baseline_data_path});
  if (!(baseline_data.is_open())) {
    return {};
  }
  std::string line;
  ...
  std::pair<std::string, std::vector<double>> baseline_name_data;
  bool first_line{true};
  while (std::getline(baseline_data, line)) {
    if (first_line) {
      baseline_name_data.first = line;
      first_line = false;
    }
    else {
      baseline_name_data.second.push_back(std::stod(line));
    }
  }

  return baseline_name_data;
}
...
std::vector<MultRunSpeedup> EvaluateImpResults::GetAvgMedSpeedupOverBaselineSubsets(MultRunData& run_output,
  ...
  const std::vector<std::pair<std::string, std::vector<unsigned int>>>& subset_str_indices) const
{
  ...
  std::vector<MultRunSpeedup> speedup_data;
  const auto baseline_run_data = GetBaselineRuntimeData(base_data_path_opt_single_thread[0]);
  if (baseline_run_data) {
    std::string speedup_header = "Speedup relative to " + std::string((*baseline_run_data).first) + " - " + std::string(data_type_str);
    ...
    for (const auto& curr_subset_str_indices : subset_str_indices) {
      ...
      speedup_header = "Speedup relative to " + std::string((*baseline_run_data).first) + " on " + std::string(curr_subset_str_indices.first) + " - " + std::string(data_type_str);
      for (unsigned int i : curr_subset_str_indices.second) {
        if (run_output[i]) {
          speedups_vect.push_back(baseline_runtimes[i] / run_output[i]->at(1).GetDataAsDouble(run_eval::kOptimizedRuntimeHeader).value());
...
std::vector<MultRunSpeedup> EvaluateImpResults:: GetAvgMedSpeedupOverBaseline(MultRunData& run_output,
  std::string_view data_type_str, const std::array<std::string_view, 2>& baseline_path_opt_single_thread) const
{
  ...
  if (baseline_run_data_single_thread) {
    std::vector<double> speedups_vect;
    const std::string speedup_header = "Single-Thread (Orig Imp) speedup relative to " + std::string((*baseline_run_data_single_thread).first) + " - " + std::string(data_type_str);
    const auto baseline_runtimesSThread = (*baseline_run_data_single_thread).second;
    for (unsigned int i=0; i < run_output.size(); i++) {
      if (run_output[i]) {
        speedups_vect.push_back(baseline_runtimesSThread[i] / run_output[i]->at(1).GetDataAsDouble(run_eval::kSingleThreadRuntimeHeader).value());
        for (auto& run_data : run_output[i].value()) {
          run_data.AddDataWHeader(speedup_header, speedups_vect.back());
        }
      }
...
MultRunSpeedup EvaluateImpResults::GetAvgMedSpeedupOptPParams(MultRunData& run_output, std::string_view speedup_header) const {
  std::vector<double> speedups_vect;
  for (unsigned int i=0; i < run_output.size(); i++) {
    if (run_output[i]) {
...
MultRunSpeedup EvaluateImpResults::GetAvgMedSpeedup(MultRunData& run_output_base, MultRunData& run_output_target,
...
MultRunSpeedup EvaluateImpResults::GetAvgMedSpeedupLoopItersInTemplate(MultRunData& run_output,
  std::string_view speedup_header) const
{
